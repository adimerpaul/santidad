<template>
  <div class="q-pa-md">
    <q-table dense :filter="filter" title="Control de unidades" :rows="data" :columns="columns" row-key="name" :rows-per-page-options="[0]">
  ...
  methods: {
    regDialog () {
      this.$q.dialog({
        ...
        this.$axios.post('unids', { nombre: data }).then(() => {
          this.$q.notify({
            color: 'green-4',
            textColor: 'white',
            icon: 'cloud_done',
    ...
    misdatos () {
      this.$axios.get('unidAll').then((res) => {
        this.data = res.data
      })
    },
    editRow (item) {
      this.$q.dialog({
        ...
        this.$axios.put('unids/' + item.row.id, { nombre: data }).then(() => {
          this.$q.notify({
            color: 'green-4',
            textColor: 'white',
            icon: 'cloud_done',
    ...
    deleteRow (item) {
      this.$q.dialog({
        ...
        this.$axios.delete('unids/' + item.row.id).then(() => {
          this.$q.notify({
            color: 'green-4',
            textColor: 'white',
            icon: 'cloud_done',
